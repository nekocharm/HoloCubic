...
static uint8_t __g_font_buf[288];//如bin文件存在SPI FLASH可使用此buff//288
...
static uint8_t *__user_font_getdata(int offset, int size){
    ...
    lv_fs_file_t font_f;
    lv_fs_res_t font_res=lv_fs_open(&font_f,"lv_font_jingnan.bin",LV_FS_MODE_RD);
    lv_fs_seek(&font_f,offset);
    font_res=lv_fs_read(&font_f,__g_font_buf,size,NULL);
    return __g_font_buf;
}
